#!/bin/bash
set -e

# This generates the input to `genblocks` to create block sets for testing small
# and large (more or less than 10 blocks) reorgs that don't involve any
# wallet-related transactions. Run this script then use `genblocks` (in
# lightwalletd) to generate the block file. This script should be run with
# the basic-reorg directory of this repository.

# All-empty blocks for the before-reorg set.
# The current height is 663200.
mkdir -p before-reorg
for i in {663150..663200}; do
    touch "./before-reorg/$i.txt"
done
genblocks -start-height 663150 -blocks-dir before-reorg/ >before-reorg.txt
rm -r before-reorg/

# There's a reorg less-than-10 back from current height, plus a bunch of new blocks.
mkdir -p after-small-reorg
for i in {663195..663250}; do
    touch "./after-small-reorg/$i.txt"
done
# Reorg at height 663195.
echo "" > ./after-small-reorg/663195.txt
genblocks -start-height 663195 -blocks-dir after-small-reorg/ >after-small-reorg.txt
rm -r after-small-reorg/

mkdir -p after-large-reorg
for i in {663180..663250}; do
    touch "./after-large-reorg/$i.txt"
done
# Reorg at height 663180.
echo "" > ./after-large-reorg/663180.txt
genblocks -start-height 663180 -blocks-dir after-large-reorg/ >after-large-reorg.txt
rm -r after-large-reorg/